...
public class GameInputManager : MonoBehaviour {
   ...
   public class EventData {
      ...
      public EventData(int mouse) { this.mouse = mouse; }
   }
   #endregion
   ...
   /// <summary>Register a mouse as one of interest.</summary>
   public static void ObserveMouseButton(int mouse) {
      if ((mouse >= 0) && (mouse < 3) && Singleton) Singleton.observedMouses.Add(mouse);
   }
   ...
   protected void Update() {
      ...
      foreach (int m in observedMouses) {
         if (Input.GetMouseButtonDown(m)) SendEvent(new EventData(m));
      }
   ...
   protected HashSet<int>     observedMouses = new HashSet<int>();
